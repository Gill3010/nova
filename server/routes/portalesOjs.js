...
router.get('/', verifyToken, (req, res) => {
  ...
  db.all(query, [], (err, rows) => {
    if (err) {
      ...
      return res.status(500).json({ success: false, error: 'Error interno al consultar portales OJS' });
    }
...
router.get('/:id', verifyToken, (req, res) => {
  ...
  db.get(query, [portalId], (err, row) => {
    if (err) {
      logger.error('Error al consultar portal OJS', { error: err.message, portalId });
...
// ──────────────────────────────────────────────────────────────
router.get('/:id/revistas', verifyToken, (req, res) => {
  const portalId = req.params.id;
  ...
  db.all(
    ...
    (err, rows) => {
      if (err) {
        logger.error('Error al consultar revistas del portal', { error: err.message, portalId });
        return res.status(500).json({ success: false, error: 'Error interno' });
      }
...
// ──────────────────────────────────────────────────────────────
router.get('/congreso/:congresoId/revistas', verifyToken, (req, res) => {
  const congresoId = req.params.congresoId;
  ...
  db.all(query, [congresoId], (err, rows) => {
    if (err) {
      logger.error('Error al consultar revistas del congreso', { error: err.message, congresoId });
      return res.status(500).json({ success: false, error: 'Error interno' });
    }
    res.status(200).json({ success: true, data: rows || [] });
  });
