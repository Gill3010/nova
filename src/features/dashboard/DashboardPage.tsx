...
export const DashboardPage: React.FC<DashboardPageProps> = ({ onClose, onEditCongress }) => {
  ...
  const [viewScope, setViewScope] = useState<'mine' | 'all'>(isAdminOrOrg ? 'mine' : 'all');
  ...
  const loadData = async (scopeToLoad = viewScope) => {
    ...
    setError(null);
    try {
      const result = await fetchDashboardData(scopeToLoad);
      setData(result);
    } catch (err: any) {
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-7xl mx-auto px-4 py-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
        <div>
          <h1 className="text-xl font-bold text-zinc-900 dark:text-white flex items-center gap-2.5">
            ...
            {viewScope === 'mine' ? 'Mis Congresos' : 'Directorio de Eventos'}
          </h1>
      ...
      {error ? (
        ...
        <>
          {isAdminOrOrg && viewScope === 'mine' ? (
            <StatCards totalCongresos={totalCongresos} totalEnvios={totalEnvios} />
          ) : null}
          
          <div className="flex flex-col gap-3 mt-4">
            <h2 className="text-lg font-bold text-slate-900 dark:text-white">
              {viewScope === 'mine' ? 'Listado de Congresos' : 'Eventos Disponibles'}
            </h2>
            {isLoading ? (
