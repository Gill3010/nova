...
export const AgendaAdmin: React.FC = () => {
  ...
  const [fotoPonente, setFotoPonente] = useState('');
  ...
  const handleEditClick = (act: Actividad) => {
    ...
    setFotoPonente(act.foto_ponente || '');
  };
            ...
            <div className={`bg-slate-50 dark:bg-slate-900/50 p-5 rounded-xl border transition-colors ${editingActividadId ? 'border-indigo-400 dark:border-indigo-600 bg-indigo-50/30' : 'border-slate-100 dark:border-slate-800'}`}>
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                ...
                <div className="flex items-center gap-4 py-2 border-b border-slate-100 dark:border-slate-800">
                  <div className="relative shrink-0 w-12 h-12 rounded-full border border-dashed border-slate-350 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 flex items-center justify-center overflow-hidden">
                    {fotoPonente ? (
                      <>
                        <img src={fotoPonente} alt="Vista previa del ponente" className="w-full h-full object-cover" />
                        <button
                          type="button"
                          onClick={() => setFotoPonente('')}
                      ...
                      <span className="text-[10px] text-slate-400 text-center px-1">Sin foto</span>
                    )}
                  </div>
                  <div className="flex flex-col gap-1 flex-1">
                    <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">
                      Foto del Ponente (Opcional)
                    </label>
                    <input
                      type="file"
                      accept="image/*"
                      id="foto-ponente-input"
                      className="hidden"
                      onChange={async (e) => {
                    ...
                    />
                    <label
                      htmlFor="foto-ponente-input"
                      className="inline-flex items-center justify-center px-2.5 py-1 border border-slate-200 dark:border-slate-700 text-xs font-medium rounded-lg text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-750 cursor-pointer text-center w-fit transition-colors"
                    >
                      Seleccionar imagen
                    </label>
                  </div>
                ...
                <div key={act.id} className="flex flex-col sm:flex-row gap-4 p-4 bg-white dark:bg-zinc-950 border border-slate-200 dark:border-slate-800 rounded-xl hover:shadow-sm transition-all group">
                  ...
                  <div className="shrink-0 flex items-center justify-center self-center sm:self-auto">
                    {act.foto_ponente ? (
                      ...
                      <div className="w-11 h-11 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white text-xs font-bold shadow-xs">
                        {act.descripcion ? getInitials(act.descripcion) : 'P'}
                      </div>
                    )}
                  </div>
