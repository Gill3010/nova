...
const getErrorMessage = (err: any): string => {
  ...
  if (typeof err === 'object') {
    const parts: string[] = [];
    if (err.status) {
      parts.push(`[HTTP ${err.status}] ${err.statusText || ''}`);
    }
    if (err.data) {
      if (typeof err.data === 'object') {
        parts.push(err.data.error || err.data.message || JSON.stringify(err.data));
      } else {
        const dataStr = String(err.data);
        parts.push(dataStr.length > 200 ? `${dataStr.substring(0, 200)}...` : dataStr);
      }
    }
  ...
  const publishAndSyncOjs = useCallback(async ({
    ...
    if (activeRole === 'ponente') {
      ...
      try {
        ...
        if (congressJson.id) {
          ...
          const pgRes = await fetch(import.meta.env.PROD ? '/api/envios' : 'http://localhost:3001/api/envios', {
            ...
            body: JSON.stringify({
              ...
              autor_email: validContributors[0]?.email || null,
          ...
          const pgData = await pgRes.json().catch(() => {
            throw new Error(`Respuesta inválida del servidor (Status: ${pgRes.status})`);
          });
          if (pgRes.ok && pgData.success) {
            addLog('success', `Envío registrado en BD local (Congreso ID ${congressJson.id})`);
          } else {
            throw new Error(pgData.error || 'Error desconocido al registrar el envío en la base de datos local');
          }
        }
        ...
        alert(`Error al enviar la ponencia: ${msg}`);
      } finally {
        setIsPublishing(false);
      }
      return;
